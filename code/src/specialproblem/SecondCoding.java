...
public class SecondCoding extends State {
	...
	public SecondCoding(Handler handler) {
		...
		restartLevel = new UIImageButton(794 - 58 - 58, 10, 48, 32, Assets.restartLevel, new ClickListener() {
			@Override
			public void onClick() {
				handler.getGame().createNewInstance(4);
				handler.getGame().readHintWordsFormed();
				handler.getGame().setHints(0);
				handler.getGame().getDisplay().setFrameInvisible(1);
				reset();
...
//			checkOutput(pr2);
	...
	public void reset() {
		result = "";
		handler.getGame().getDisplay().getTextArea(1).setText("#!/bin/bash");
		disableProceed();
	}
