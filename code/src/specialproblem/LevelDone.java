...
public class LevelDone extends State {
	...
	public void render(Graphics g) {
		...
		if(isAccomplished == 0) {
			if(level == 10) {
				Text.drawString(g, "CONGRATULATIONS!", handler.getGame().getWidth() / 2,
						200, true, Color.BLACK, Assets.courier);
				
				Text.drawString(g, "You have now finished all the levels! Now you can play all the levels in order to", handler.getGame().getWidth() / 2,
						300, true, Color.BLACK, Assets.garamonditalic);
				Text.drawString(g, "further improve and hone your skills in shell scripting, and be a better programmer.", handler.getGame().getWidth() / 2,
						325, true, Color.BLACK, Assets.garamonditalic);
				Text.drawString(g, "Happy coding!", handler.getGame().getWidth() / 2,
						350, true, Color.BLACK, Assets.garamonditalic);
			}
			else {
