...
public class Crossword extends State {
	...
	public Crossword(Handler handler, int level, int[] lettersIndices, String[] words, String[] defs) {
		...
		proceed = new UIImageButton(handler.getGame().getWidth() / 2 + 5, 510, 0, 0, Assets.proceed, new ClickListener() {
			@Override
			public void onClick() {
...
//				handler.getGame().crosswords[level].setUIManager();
		...
		for(int i = 0; i < 5; i++) {
			for(int j = 0; j < 5; j++) {
				CrosswordTile c = new CrosswordTile((float) (325 + 50 * i), (float)((275 + 50 * j) - 50), 50, 50, Assets.alphabet[lettersIndices[5 * i + j]], lettersIndices[5 * i + j], this);
				uiManager.addObject(c);
				crosswordTiles.add(c);
			}
		}
		
		uiManager.addObject(new UIImageButton(700, 200, 32 * 3, 32 * 2, Assets.submit_btn, new ClickListener() {
			@Override
			public void onClick() {
...
//				}
		...
		uiManager.addObject(new UIImageButton(700, 264 + 54, 32 * 3, 32 * 2, Assets.clear_btn, new ClickListener() {
			@Override
			public void onClick() {
				for(CrosswordTile c : crosswordTiles) {
					if(c.isSelected() == true) {
		...
		uiManager.addObject(new UIImageButton(900 / 2 - 96 - 5, 510, 32 * 3, 32 * 2, Assets.shuffle_btn, new ClickListener() {
			@Override
			public void onClick() {
				// clear first
				for(CrosswordTile c : crosswordTiles) {
...
//				closeHintWindow.updateBounds();
		...
		uiManager.addObject(new UIImageButton(700, 264 + 54 + 64 + 54, 32 * 3, 32 * 2, Assets.bonusWords, new ClickListener() {
			@Override
			public void onClick() {
				showHintWordsWindow = true;
				closeHintWindow.setWidth(48);
...
//		g2.drawRoundRect(290, -10, 620, 290 + 900, 20, 20);
//		g2.setStroke(oldStroke);
		
		
		g.drawImage(Assets.letterBox, 300, 250 - 50, null);
...
//		System.out.println(this.stringFormed);
		...
		for(int i = 0; i < words.length; i++) {
			if(contains(wordsCreatedIndices, i) == false) {
				String underscoreText = "";
				for(int j = 0; j < words[i].length(); j++) {
					underscoreText = underscoreText + "_ ";
				}
				
				Text.drawString(g, underscoreText, 5, defStartLine + (i * 130) - 25, false, Color.RED, Assets.garamondbold2);
			}
		}
