...
public class FourthCoding extends State {
	...
	public FourthCoding(Handler handler) {
		...
		restartLevel = new UIImageButton(794 - 58 - 58, 10, 48, 32, Assets.restartLevel, new ClickListener() {
			@Override
			public void onClick() {
				handler.getGame().createNewInstance(8);
				handler.getGame().readHintWordsFormed();
				handler.getGame().setHints(0);
				handler.getGame().getDisplay().setFrameInvisible(3);
				reset();
...
//			checkOutput(pr2);
	...
	public void reset() {
		result = "";
		handler.getGame().getDisplay().getTextArea(3).setText("#!/bin/bash");
		disableProceed();
	}
